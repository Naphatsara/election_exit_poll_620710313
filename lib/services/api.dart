...
import 'package:election_exit_poll_620710313/models/api_result.dart';

import 'package:http/http.dart' as http; // ต้องไป add library ส่วน dependency ใน pubspec.yaml
...
class Api {
  ...
  Future<dynamic> submit(String endPoint, Map<String, dynamic> params,) async {
    ...
    final response = await http.post(
      url,
      headers: {'Content-Type': 'application/json', 'id': '620710313'},
    ...
    if (response.statusCode == 200) {
      ...
      print('RESPONSE BODY SUBMIT: $jsonBody'); // print map from api (status / mes / data)
  ...
  Future<dynamic> fetch(String endPoint, {Map<String, dynamic>? queryParams,}) async {
    ...
    final response = await http.get(url, headers: {'id': '620710313'});

    if (response.statusCode == 200) {
      // แปลง text ที่มีรูปแบบเป็น JSON ไปเป็น Dart's data structure (List/Map)
      Map<String, dynamic> jsonBody = json.decode(response.body);

      print('RESPONSE BODY FETCH: $jsonBody'); // print map from api (status / mes / data)
